...
This guide will help you migrate Amplify JavaScript v5's Storage APIs to the new Amplify JavaScript v6's Storage APIs.

## Deprecated Options

<Callout>Many input and output properties have been deprecated in favor of alternatives with a better developer experience. If you have identified a use case that you feel isn't covered in this migration guide, feel free to open a [GitHub issue](https://github.com/aws-amplify/amplify-js/issues/new?assignees=&labels=pending-triage&projects=&template=2.feature_request.yaml).</Callout>

The input options below have been removed in Amplify JavaScript v6 from Storage APIs:
...
- `track`: _Auto-tracking Storage events in pinpoint has been removed in V6. You can continue to track events [using the Analytics category](/[platform]/build-a-backend/more-features/analytics/record-events/). See [Tracking storage events](/[platform]/build-a-backend/storage/storage-v5-to-v6-migration-guide/#tracking-storage-events) for migration details.__
...
- `$metadata`: _Information like `httpStatusCode` and `attempts` will be provided in an upcoming release via debug messages using [the Console Logger](/[platform]/build-a-backend/utilities/console-logger/)._
...
>Note: We have remapped all capitalized input and output keys to "camelCase" in Amplify JavaScript v6.
...
- The `resumable` option has been removed from `config` as all upload tasks are resumable by-default in Amplify JavaScript v6. You can find the `pause`, `resume`, and `cancel` functions on `uploadData`'s return value. See [the v6 Storage documentation](/[platform]/build-a-backend/storage/upload/#pause-and-resume-upload) for details.
  ...
  <Columns columns={2}>
    <div>
      **V5**
      ...
      ```
      // Assumes default provider 'AWSS3': custom provider Deprecated in V6
      key: string
      options: {
        level?: 'public' | 'protected' | 'private';
        provider?: 'AWSS3';
...
The `track` option has been removed from Storage and its APIs in V6. Previously, in V5, enabling this option would automatically send analytics events to Pinpoint following the success or failure of Storage API calls. The analytic events sent were defined by Amplify and therefore not configurable and, in V6, this options has been removed but you can continue to track Storage API results in a more predictable and configurable way [using the Analytics category](/[platform]/build-a-backend/more-features/analytics/record-events/).

The table below lists the analytics events previously associated to V5 APIs and the V6 APIs they should now be associated with instead.
